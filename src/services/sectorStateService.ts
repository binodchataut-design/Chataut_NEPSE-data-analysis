...
export class SectorStateService {
  ...
  public static getSectorState(symbolOrSectorId: string, asOfDate?: string): CurrentSectorState {
    ...
    if (!sector) {
      // Check if it's a company symbol
      const company = normalizedCompanies.find(c => c.symbol.toUpperCase() === cleanKey);
      if (company) {
        sector = normalizedSectors.find(s => s.id === company.sector_id);
      }
    }
    ...
    // Sector Breadth
    const sectorCompanies = normalizedCompanies.filter(c => c.sector_id === sector!.id);
