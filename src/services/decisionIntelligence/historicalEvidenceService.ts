...
import { getNormalizedStockBars, normalizedCompanies } from '../../data/normalizedMasterData';
...
export class HistoricalEvidenceService {
  ...
  public static getHistoricalComparables(
    ...
    // Scan liquid NEPSE stocks for historical instances
    const symbolsToScan = normalizedCompanies.map(c => c.symbol);
    ...
    for (const sym of symbolsToScan) {
      ...
      for (let i = 15; i < bars.length - 1; i++) {
        const barDate = (bars[i] as any).date || bars[i].timestamp;
        if (barDate >= asOfDate) {
        ...
        if (similarity >= 0.70) {
          ...
          for (const h of horizons) {
            ...
            const targetDate = (targetBar as any).date || targetBar.timestamp;
            ...
            if (targetDate <= asOfDate && targetIdx > i) {
              ...
              const availableForward = bars.slice(i + 1).filter(b => ((b as any).date || b.timestamp) <= asOfDate);
  ...
  public static calculateMultiHorizonProbabilities(
    ...
    for (const h of horizons) {
      ...
      distributions[h] = {
        ...
        downsidePercentile10: dist.percentile25 * 1.5, // Conservative estimate
        downsidePercentile25: dist.percentile25,
        upsidePercentile75: dist.percentile75,
        upsidePercentile90: dist.percentile75 * 1.5,
