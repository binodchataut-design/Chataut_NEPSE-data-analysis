...
import { getCachedCompany } from '../data/liveBarsCache';
...
export class ListingLifecycleService {
  ...
  public static getLifecycle(symbol: string): ListingLifecycle | null {
    ...
    // Fallback to CompanyMaster if available (using cached company in SUPABASE mode, fallback to normalizedCompanies)
    const cachedComp = getCachedCompany(sym);
    const comp = cachedComp || normalizedCompanies.find(c => c.symbol === sym);
    if (comp) {
      return {
        symbol: comp.symbol,
        ...
        suspensionPeriods: [],
        currentStatus: (comp.status as LifecycleStatus) || 'ACTIVE',
        lifecycleStatusSource: 'NEPSE_OFFICIAL'
      };
    }
