        ...
        <div className="lg:col-span-2 bg-[#111722] border border-slate-800 rounded-lg p-4">
          <div className="flex items-center justify-between pb-3 border-b border-slate-800/80 mb-3">
            ...
            <div className={`px-2.5 py-0.5 rounded text-xs font-mono font-bold border ${getRegimeColor(breadth?.marketRegime)}`}>
              REGIME: {breadth?.marketRegime || 'BULLISH'} ({breadth?.regimeScore || 76}/100)
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
            ...
            <div className="bg-[#0b0f17] p-3 rounded border border-slate-800/80">
              ...
              <div className="flex items-baseline gap-2 mt-1">
                <span className="text-lg font-bold font-mono text-emerald-400">{breadth?.advancers}</span>
                <span className="text-xs text-slate-500 font-mono">/</span>
                <span className="text-lg font-bold font-mono text-rose-400">{breadth?.decliners}</span>
                <span className="text-xs text-slate-500 font-mono">({breadth?.unchanged} unch)</span>
              </div>
              {/* Ratio Bar */}
              <div className="w-full h-1.5 bg-rose-500/40 rounded-full mt-2 overflow-hidden flex">
                <div
                  ...
                  style={{
                    width: `${((breadth?.advancers || 164) / ((breadth?.advancers || 164) + (breadth?.decliners || 72))) * 100}%`,
                  }}
                />
              </div>
              <div className="text-[10px] font-mono text-slate-400 mt-1 flex justify-between">
                <span>Ratio: {breadth?.advanceDeclineRatio.toFixed(2)}:1</span>
                <span>{((breadth?.advancers || 164) / 2.44).toFixed(0)}% Advancing</span>
              </div>
            </div>
            ...
            <div className="bg-[#0b0f17] p-3 rounded border border-slate-800/80">
              ...
              <div className="text-lg font-bold font-mono text-slate-100 mt-1">
                {breadth?.aboveSma20Percent}% <span className="text-xs font-normal text-slate-400">(&gt;20d)</span>
              </div>
              <div className="text-[11px] font-mono text-slate-400 mt-1">
                Above 50 SMA: <span className="text-slate-200 font-semibold">{breadth?.aboveSma50Percent}%</span>
              </div>
              <div className="text-[11px] font-mono text-slate-400">
                Above 200 SMA: <span className="text-slate-200 font-semibold">{breadth?.aboveSma200Percent}%</span>
              </div>
            </div>
