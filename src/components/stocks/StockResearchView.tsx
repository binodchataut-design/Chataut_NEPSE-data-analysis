...
export function StockResearchView({
  ...
  useEffect(() => {
    ...
    ]).then(([stk, cnd, ind, tSc, fMet, fSc, bConc, stps]) => {
      setStock(stk);
      setCandles(cnd);
      setIndicators(ind);
      setTechScore(tSc);
  ...
  if (!stock || !indicators || !techScore || !fundMetrics || !fundScore) {
    return (
    ...
    <div className="p-6 space-y-5 max-w-[1600px] mx-auto font-sans">
      ...
      <div className="bg-[#111722] border border-slate-800 rounded-lg p-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3 pt-4 font-mono text-xs">
          ...
          <div className="bg-[#0b0f17] p-2.5 rounded border border-slate-800">
            ...
            <span className="text-[10px] text-slate-400">Top Buyer: Broker #{brokerConcentration?.topBuyerBroker}</span>
          </div>
      ...
      <div className="flex items-center gap-2 border-b border-slate-800 pb-1">
        {[
          ...
          { id: 'DECISION', label: 'Phase 4B Decision Intel' },
        ].map(tab => (
          <button
            key={tab.id}
