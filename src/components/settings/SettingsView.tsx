...
export function SettingsView() {
  ...
  const handleSetDataMode = (newMode: DataSourceMode) => {
    setDataMode(newMode);
    dataService.setMode(newMode).catch(err => {
      console.error('[SettingsView] Failed to switch data mode:', err);
    });
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-[#111722] border border-slate-800 rounded-lg p-5 space-y-4">
            ...
            <div className="p-3.5 bg-[#0c1018] rounded border border-slate-800 space-y-3">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="text-[11px] text-slate-400 mt-1">
                    {dataMode === 'SUPABASE'
                      ? 'Connected to real Supabase NEPSE data warehouse (5-year historical prices & official index records).'
                      : dataMode === 'MOCK_DATA'
                      ? 'Using high-fidelity offline sample data for isolated architectural testing.'
                      : 'Configured for live external API gateway.'}
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 pt-2 border-t border-slate-800/80">
                <button
                  onClick={() => handleSetDataMode('SUPABASE')}
                ...
                >
                  SUPABASE (Default Real Data)
                </button>
                <button
                  onClick={() => handleSetDataMode('MOCK_DATA')}
                  className={`px-3 py-1.5 rounded text-xs font-bold transition-colors ${
                    dataMode === 'MOCK_DATA'
