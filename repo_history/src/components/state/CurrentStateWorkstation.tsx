...
import { getCachedBars, getCachedCompanies } from '../../data/liveBarsCache';
...
export function CurrentStateWorkstation({
  ...
  const availableDates = useMemo(() => {
    const bars = getCachedBars(selectedSymbol);
    const dates = bars.map(b => b.date).sort((a, b) => b.localeCompare(a));
    return dates;
  }, [selectedSymbol, dataMode]);

  // Companies list (all 437 in Supabase, 17 in mock)
              ...
              >
                {availableCompanies.map(c => (
                  <option key={c.symbol} value={c.symbol} className="bg-slate-900 text-slate-200">
                    {c.symbol} - {(c.company_name || c.symbol).substring(0, 22)}
                  </option>
                ))}
              </select>
            </div>
      ...
      {snapshot.stateAvailability && !snapshot.stateAvailability.isAvailable ? (
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="space-y-1.5 flex-1">
              ...
              <p className="text-sm text-slate-300">
                The Current State Engine cannot evaluate point-in-time states for <strong className="font-mono text-cyan-300">{selectedSymbol}</strong> because the live upstream data provider is unavailable or unauthenticated.
              </p>
            </div>
          </div>
            ...
            <button
              onClick={() => {
                dataService.setMode('MOCK_DATA').catch(err => {
                  console.error('[CurrentStateWorkstation] Failed to switch mode to MOCK_DATA:', err);
                });
              }}
        ...
        <div
          onClick={() => openExplanation(
            ...
            'Evaluated deterministically across trend moving averages, momentum indicators, volume/price ratios, and data quality audits.'
          )}
          className="bg-[#0b101b] border border-slate-800 hover:border-slate-700 rounded-xl p-4 cursor-pointer transition-all hover:shadow-lg group"
        >
          <div className="flex items-center justify-between mb-3">
